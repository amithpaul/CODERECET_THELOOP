...
def analyze_innovation(text: str) -> dict:
    """
    Analyzes a single piece of text to generate a summary, tags, and sentiment.
    Returns a dictionary.
    """
    print(f"Processing: '{text[:50]}...'")
    prompt = f"""
    Analyze the following innovation/suggestion from a citizen in Kerala.
...
if __name__ == "__main__":
    ...
    for innovation in innovations_data:
        ...
        result['original_idea'] = innovation  # Add original text for context
        all_results.append(result)
        
    print("\n--- AI Analysis Complete ---")

    # --- Save results to a JSON file ---
    json_filepath = "innovations.json"
    with open(json_filepath, 'w', encoding='utf-8') as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)
    print(f"✅ Results saved to {json_filepath}")

    # --- Save results to a CSV file ---
    csv_filepath = "innovations.csv"
    headers = ['original_idea', 'summary', 'tags', 'sentiment']
    with open(csv_filepath, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=headers)
        writer.writeheader()
        # Convert the list of tags into a single string for CSV
